...
/**
 * <p>
 *     in addition to 另外，加之，除...之外    specify具体说明，明确规定
 ...
 *  源记录由SourceRecords生成并传递给kafka connect以在kafka中存储。
 *  除了ConnectRecord 中指定数据存储在kafka中的位置的标准字段外，他们还包括sourcePartition和sourceOffset
 * </p>
 * <p>
 ...
 *  sourcePartition代表记录来自单个输入sourcePartition（例如，文件名，表名或主题分区）。sourceOffset标示该
 *  sourcePartition中可用于恢复数据消耗的位置
 *
 *  resume摘要，概述
 *
 * </p>
 ...
 * 这些值可以是任意结构，应该使用org.apache.kafka.connect.data 或者是原始值
 * 例如，数据库连接器可以将sourcePartition制定包含db：数据库名称，表：表名，sourceOffset作为包含行时间戳的长字符串
 * </p>
 */
