...
/**
 * <p>
 * Base class for records containing data to be copied to/from Kafka.包含要复制到Kafka或从Kafka复制的数据的记录的基类
 * This corresponds closely to  这与kafka的ProducerRecord 和 ConsumerRecord类密切相关，并保存两个类都可以使用的数据
 ...
 * 源和汇（主题，kafka分区，键，值）尽管这两个实现都包含了偏移量的概念，但这里不包括，因为他们的类型不同
